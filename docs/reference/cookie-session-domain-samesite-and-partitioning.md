...
sequenceDiagram
    ...
    U->>B: ログイン操作
    B->>A: POST /login<br/>本文にログインIDとパスワード
...
セッションストアの代表的な保存場所は、次のとおりです。

| 保存場所 | 保存方法の例 | 長所 | 注意点・主な用途 |
|---|---|---|---|
| アプリケーションプロセスのメモリ | Goの`map`などへセッションを保存する | 構成が単純で高速。学習や試作で使いやすい | プロセスの再起動で消える。複数プロセス・複数サーバー間ではそのまま共有できない |
| Redisなどのインメモリデータストア | セッションIDをキー、セッション情報を値として保存する | 高速で、有効期限を設定しやすい。複数のアプリケーションサーバーから共有できる | Redis自体の運用、障害対策、アクセス制御が必要になる。本番の共有セッションストアでよく利用される |
| RDBなどのデータベース | セッション用テーブルへID、ユーザーID、有効期限などを保存する | 既存DBを利用でき、永続化や検索を行いやすい | リクエストごとのDB負荷、期限切れデータの削除、性能設計が必要になる |

本番環境で複数のアプリケーションサーバーから同じセッションを使う場合は、共有できるRedisやDBなどがよく利用されます。

どれが常に正解というわけではありません。
学習環境か本番環境か、アプリケーションサーバーの数、必要な性能、再起動時にセッションを維持するかによって選びます。

### 最初のログインリクエストでは何を送るのか

ここでは、次の二つを分けて考えることが重要です。

1. ログインするために利用者が入力するデータ
2. ブラウザが保存済みCookieから自動で付けるデータ

通常、ログインID、パスワード、ワンタイムパスワードなどはCookieではありません。
アプリケーションが定めたログインAPIのリクエスト本文などで送ります。

#### 誰がデータを決めるのか

| データ | 主に決める人・機能 | どこで指定するか | 送信方法の例 |
|---|---|---|---|
| ログインID、パスワード、ワンタイムパスワード | アプリケーションの認証設計者、バックエンド開発者 | API仕様、Goのリクエスト構造体、ログインハンドラー | JSONやHTMLフォームのリクエスト本文 |
| 画面で入力した値をどの形式で送るか | フロントエンド開発者 | HTMLフォーム、JavaScriptの`fetch`など | API仕様に合わせてJSONやフォームデータを作る |
| CSRFトークンなどの事前データ | バックエンドとフロントエンドのセキュリティ設計 | ログイン画面、レスポンス、フォーム、HTTPヘッダーなど | リクエスト本文または専用ヘッダー。方式によってCookieも併用する |
| すでに保存されているCookie | 元の値と属性はアプリケーション、送信可否はブラウザ | 過去の`Set-Cookie`と現在のリクエスト条件 | ブラウザが`Cookie`ヘッダーへ自動で付ける |
| 新しいセッションID | バックエンドのセッション管理処理 | 認証成功後にサーバーで生成する | レスポンスの`Set-Cookie`でブラウザへ渡す |

例えば、Goのバックエンドでは、次のようなリクエスト構造体やハンドラーが受け付ける項目を定めます。
...
フロントエンドは、このAPI仕様に合わせて`email`と`password`を含むJSONを作ります。
ブラウザがGoの構造体を見てデータを決めるのではなく、開発者がバックエンドとフロントエンドの共通仕様として設計します。

アプリケーションごとに、ログインIDを使うかメールアドレスを使うか、パスワード以外に何を要求するかは異なります。
その意味で「ログイン時に送るデータはアプリケーションごとに違う」という仮説は正しいです。

ただし、新しく発行するセッションIDを、ブラウザが最初のログインリクエストで決めて送るわけではありません。
セッション固定攻撃などを防ぐためにも、認証成功時にはサーバー側が推測困難な新しいセッションIDを発行します。
...
sequenceDiagram
    actor U as 利用者
    participant B as ブラウザ内の画面・JavaScript
    participant A as アプリケーションサーバー
    participant S as セッションストア

    U->>B: ログインIDとパスワードを入力
    Note over B: API仕様に合わせてリクエスト本文を作成
    B->>A: POST /login<br/>ログイン情報を本文で送信
    Note over B: 保存済みCookieがあれば条件に従って自動送信
    A->>A: ログイン情報を検証
    A->>S: 新しいsession_idとuser_idを保存
    A-->>B: Set-Cookieで新しいsession_idを発行
    Note over B: 新しいsession_idを保存
    B->>A: GET /mypage<br/>新しいsession_idをCookieで送信
```

HTTPメッセージで見ると、概ね次のようになります。

```http
POST /login HTTP/1.1
Host: app.example.com
Content-Type: application/json
Cookie: csrf_session=prelogin456

{"email":"user@example.com","password":"..."}
```

この`Cookie`ヘッダーは、ログイン前の画面などですでに発行されていたCookieがある場合の例です。
そのアプリケーションへ初めてアクセスし、保存済みCookieがなければ、この行はありません。

認証に成功すると、サーバーが新しいセッションIDを発行します。
...
この例はHTTPメッセージの構造を示したものです。
実際のログイン情報は必ずHTTPSで送信し、パスワードやセッションIDをアプリケーションログへ記録しないようにします。

#### ブラウザを識別する共通の値が自動送信されるのか

ブラウザが、すべてのアプリケーションに共通する永続的な「ブラウザ固有ID」を、Cookieとして自動生成して送るわけではありません。

- サーバーからはIPアドレスや`User-Agent`などが観察できる
- ただし、これらは変化したり複数利用者で共有されたりするため、確実な利用者識別には使えない
- アプリケーションが匿名カートや端末識別を必要とする場合は、アプリケーション側が識別子を生成して、事前にCookieなどへ保存させることがある
- その識別子も削除・複製され得るため、それだけを安全な本人認証として信用しない
...
| 最初のログイン時にブラウザが送るCookieデータは誰が決めるのか | ログインIDやパスワードなどはCookieではなく、アプリケーションのAPI仕様で決めたリクエストデータ。Cookieとして自動送信されるのは、すでに保存されていて条件に一致するものだけ | ログイン用リクエスト本文とCookieヘッダーは役割が異なる |
| ブラウザが自ら生成した識別値を最初のログインで送るのか | 新しいセッションIDは認証成功後にサーバーが生成する。ブラウザ共通の永続的な固有IDが自動送信されるわけでもない | セッションIDの発行者はアプリケーションであり、ブラウザは受け取った値を保存・返送する |
...
| 最初のログイン | ブラウザが新しいセッションIDを作って送る | APIで定めたログイン情報を本文で送り、認証成功後にサーバーが新しいセッションIDを発行する |
...
- [ ] ログイン用リクエスト本文と、自動送信されるCookieを区別できる
- [ ] 新しいセッションIDを誰が、いつ発行するか説明できる

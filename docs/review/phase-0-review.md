...
`curl -v`と`curl -i`によるGET通信に加え、ブラウザのNetworkタブによる通信の観察と、その結果の記録を確認できました。
特に、TLS、HTTP/2、リクエスト、レスポンスを分けて調べた点はよいです。

一方、クエリ付きGETとPOSTは「やったこと」に列挙されていますが、実行内容や観察結果が記録されていません。
...
| 完了条件 | 判定 | レビュー結果 |
|---|---|---|
| リクエストのメソッド、パス、クエリ、ヘッダー、ボディを説明できる | 一部完了 | メソッド、パス、ヘッダー、空ボディは説明済み。クエリの実行内容と観察結果がない |
...
| 観察した通信の各要素を記録した | 一部完了 | curlによるGETとレスポンス、ブラウザの観察結果は記録済み。クエリとPOSTの記録が不足 |
...
```text
1. Networkタブの記録に実際のステータスコード、ヘッダー、Cookieの確認結果を補う
  ↓
2. クエリ付きGETを観察する
  ↓
3. POSTを観察してGETと比較する

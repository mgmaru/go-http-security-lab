...
`curl -v`と`curl -i`によるGET通信の観察は、十分にできています。
特に、TLS、HTTP/2、リクエスト、レスポンスを分けて調べた点はよいです。

一方、ブラウザのNetworkタブ、クエリ付きGET、POSTの観察が記録されていません。
...
| 完了条件 | 判定 | レビュー結果 |
|---|---|---|
| リクエストのメソッド、パス、クエリ、ヘッダー、ボディを説明できる | 一部完了 | メソッド、パス、ヘッダー、空ボディは説明済み。クエリの観察がない |
| レスポンスのステータスコード、ヘッダー、ボディを説明できる | 完了 | 3つを分けて説明できている |
| GETとPOSTの違いを説明できる | 未確認 | GETだけが記録されている |
| Networkタブで通信を確認した | 未確認 | 実施記録がない |
| `curl -v`と`curl -i`を使用した | 完了 | コマンド、結果、観察内容が記録されている |
| 観察した通信の各要素を記録した | 一部完了 | GETとレスポンスは記録済み。クエリ、POST、ブラウザの記録が不足 |
...
Phase 0では、curlだけでなくブラウザから発生する通信も観察します。
最低1件について、次を記録してください。
...
現在のリクエストパスは`/`だけなので、クエリを観察できていません。
...
リクエストターゲット
/search?name=pikachu&limit=10
```

クエリ付きGETを送り、パスとクエリを別々に説明してください。

### 4.3 GETとPOSTの比較

POSTの実行記録がないため、GETとの違いを確認できません。
...
- [ ] ブラウザのNetworkタブで1件以上の通信を観察した
- [ ] Networkタブの観察結果を進捗メモへ記録した
- [ ] クエリ付きGETを実行した
- [ ] パスとクエリを分けて説明した
- [ ] ボディ付きPOSTを実行した
- [ ] GETとPOSTのリクエストを表で比較した
- [ ] Cookieまたは`Set-Cookie`を確認した。確認できなかった場合は、その事実を記録した
